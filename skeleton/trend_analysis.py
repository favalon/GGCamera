...
def get_focus_max_change(focus_seq):
    frames = focus_seq.shape[0]
    dist_diff = np.zeros((frames - 1))
    for i in range(frames - 1):
        diff = np.linalg.norm(focus_seq[i + 1] - focus_seq[i])
        dist_diff[i] = diff

    dis_diff_max = np.max(dist_diff)
    dist_diff_average = np.average(dist_diff)

    return dist_diff_average
