...
def _plot(frames, generate_seq, real_seq, pc, row, col, plot_i):
    axis = ['X', 'Y', 'Z']
    ...
    plt.plot(frames, diff_acl_val, c='darkviolet', label="accumulate")
    plt.legend()
...
def exp1_seq_dis_diff(generate_seq, real_seq):
    frames = [i for i in range(real_seq.shape[0])]

    # abs diff plot
    fig = plt.figure(1, figsize=(16, 7))
    # x
    _plot(frames, generate_seq, real_seq, pc=(0, 0), row=3, col=2, plot_i=1)
    # y
    _plot(frames, generate_seq, real_seq, pc=(0, 1), row=3, col=2, plot_i=3)
    # z
    _plot(frames, generate_seq, real_seq, pc=(0, 2), row=3, col=2, plot_i=5)
    plt.show()

    fig = plt.figure(2, figsize=(16, 7))
    # rx
    _plot(frames, generate_seq, real_seq, pc=(1, 0), row=3, col=2, plot_i=1)
    # ry
    _plot(frames, generate_seq, real_seq, pc=(1, 1), row=3, col=2, plot_i=3)
    # rz
    _plot(frames, generate_seq, real_seq, pc=(1, 2), row=3, col=2, plot_i=5)
    plt.show()
...
def main(path, max_frame=90):
    ...
    exp1_seq_dis_diff(generate_seq[:max_frame, :, :], real_seq[:max_frame, :, :])
